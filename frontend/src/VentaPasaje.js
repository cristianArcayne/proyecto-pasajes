...
const FormularioCompra = () => {
  ...
  }, [metodoPago, paypalLoaded, precioTotal]);
  ...
  // Enviar datos de reserva y pasar a pago
  const iniciarPago = (e) => {
    e.preventDefault();
  ...
  // Registrar compra y guardar pasaje
  const procesarCompraFinal = async (refPayPal = null) => {
  ...
  // Convertir a PNG y enviar por WhatsApp
    ...
    <div style={styles.container}>
      ...
      {vista === 'ticket' && ticketActivo && (
        <div style={styles.ticketWrapper}>
          ...
          <TarjetaEmbarque ticketActivo={ticketActivo} styles={styles} />
        </div>
      )}

      {/* Vista de recuperacion de boleto */}
      {vista === 'recuperar' && (
        <RecuperacionBoleto
          styles={styles}
          onSelectTicket={(p) => {
